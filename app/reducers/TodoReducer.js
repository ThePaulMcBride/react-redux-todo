...
function toggleCompleted(state, itemId) {
    ...
    const updatedItem = state.get('todos').get(itemIndex).update('status', status => status === 'active' ? 'completed' : 'active');
...
function addItem(state, text) {
    ...
    return state.update('todos', (todos) => todos.push(newItem)).set('input', '');
}
